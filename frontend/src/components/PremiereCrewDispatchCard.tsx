    ...
    <div className="bg-white border border-gray-200/80 rounded-2xl p-5 subtle-card-shadow flex flex-col justify-between h-full">
      <div>
        ...
        <div className="grid grid-cols-2 gap-2.5 mt-3.5">
          ...
          >
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-1.5">
                <span className="text-[10px] font-mono font-bold px-1.5 py-0.5 rounded bg-sky-100 text-sky-900">
                  KEY GRIP
                </span>
                <span className="text-[11px] font-bold text-gray-800">
                  Infra Rigger
        ...
        {/* Quick Stats Grid: MTTR & Churn Saved */}
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5 mt-2.5">
          ...
          <div className="p-2.5 bg-gray-50 rounded-xl border border-gray-200/60">
            ...
            <span className="text-sm font-bold text-emerald-600 mt-0.5 block">
              {latestInvestigation?.estimated_subscriber_loss_prevented ? "217,520 Protected" : "$1,450,000"}
            </span>
        ...
        {latestInvestigation?.rollback_status === "EXECUTED" && (
          <div className="mt-2.5 p-2.5 bg-red-50 rounded-xl border border-red-200 text-xs">
            <span className="font-bold text-red-800 text-[11px] block mb-1">
        ...
        {/* Official MCP Toolchain Strip (Light Format) */}
        <div className="mt-2.5 p-2.5 bg-gray-50 rounded-xl border border-gray-200/60 text-gray-800">
          <div className="flex items-center justify-between mb-1.5">
            <span className="text-[10px] font-mono font-bold tracking-wider text-gray-500 uppercase">
              Official Grafana MCP Toolchain
            </span>
            <span className="text-[9px] font-mono px-1.5 py-0.2 rounded bg-white text-gray-700 border border-gray-200">
              stdio JSON-RPC
            </span>
          </div>
          <div className="flex flex-wrap gap-1">
            {mcpTools.map((tool, idx) => (
              <span
                key={idx}
                className="inline-flex items-center gap-1 px-1.5 py-0.5 rounded bg-white border border-gray-200 text-[9px] font-mono text-gray-700"
              >
      ...
      <div className="mt-3 pt-2.5 border-t border-gray-100">
        <button
          ...
          className="w-full flex items-center justify-center py-2.5 px-4 rounded-xl bg-slate-900 hover:bg-slate-800 text-white text-xs font-bold transition-all disabled:opacity-50 active:scale-[0.99] shadow-sm"
        >
          <span>
            {isInvestigating
              ? "Executing Official Grafana MCP Toolchain..."
              : "Dispatch SRE Remediation (Autonomous Failover)"}
          </span>
        </button>
      </div>
    </div>
